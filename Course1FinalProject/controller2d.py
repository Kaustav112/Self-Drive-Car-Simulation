...
class Controller2D(object):
    ...
    def update_controls(self):
        ...
        self.vars.create_var('cross_track_deadband', 0.005)
        ...
        if self._start_control_loop:
            ...
            v_error_rate_of_change      = self.vars.v_error / (t - self.vars.t_prev)
            # cap the integrator sum to a min/max
            #self.vars.v_error_integral = np.fmax(np.fmin(self.vars.v_error_integral, self.vars.integrator_max), self.vars.integrator_min)
            ...
            steer_output = heading_error + np.arctan(self.vars.kp_heading * crosstrack_sign * crosstrack_error / (v))
